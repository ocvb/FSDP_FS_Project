...
export default function Users({ postSnackbar }) {
    ...
    const { data: eventsData, isFetching, isError, refetch: refetchEvents } = useQuery({
        queryKey: ['events'],
    ...
    function EditToolbar() {
        ...
        const handleClick = () => {
            ...
            const id = Math.max(0, ...eventsData?.data.map((row: any) => row.id)) + 1;
        ...
        return (
            <GridToolbarContainer sx={{
                ...
                <IconButton onClick={() => { refetchEvents(), postSnackbar({ children: 'Events refreshed', severity: 'success' }) }} >
    ...
    const columns = [
        ...
        },
        {
            ...
            type: 'date',
            width: 130,
            renderCell: (params: { formattedValue: string | number | boolean | Date | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined; }) => {
                return (
                    <Tooltip title={params.formattedValue}>
                        <span>{new Date(params.formattedValue as string).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    </Tooltip>
                )
            },
            valueGetter: (params) => {
