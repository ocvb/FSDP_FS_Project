...
import { Stack, TextField, IconButton, Alert, FormControl, Box } from '@mui/material';
...
export default function Support({ postSnackbar }: SupportProps) {
  ...
  const { data: supportData, isFetching, isError, refetch: refetchSupport } = useQuery({
    queryKey: ['support'],
  ...
  const createSupportMutation = useMutation({
    ...
    mutationFn: async (data: SelectedRow) => {
      const r = await axios.post('http://localhost:3001/api/support', data, {
        headers: {
  ...
  const updateSupportMutation = useMutation({
    mutationKey: ['support'],
    mutationFn: async (data: SelectedRow) => {
      const r = await axios.put(`http://localhost:3001/api/support/${data.id}`, data, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const deleteSupportMutation = useMutation({
    mutationKey: ['support'],
    mutationFn: async (data: SelectedRow) => {
      const r = await axios.delete(`http://localhost:3001/api/support/${data.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
      ...
      >
        <Box display={'flex'} flexDirection={'row'} gap={'0.6rem'}>
          <Button
            ...
            onClick={() => {
              refetchSupport().catch(() => console.log('Error refreshing'));
              postSnackbar({
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 1,
      editable: false,
      renderCell: (params: { value: string }) => <ExpandableCell value={params.value} />,
    },
    {
      field: 'createdAt',
  ...
  const handleOpenEditModal = (id: number) => {
    const data = supportData?.find((row: SelectedRow) => row?.id === id) as SelectedRow;
  ...
  return (
    <>
      ...
      >
        ...
        >
          {openEditModal ? `Update Support Request` : 'Add Support Request'}
        </p>
